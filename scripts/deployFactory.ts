...
export async function run(provider: NetworkProvider) {
    ...
    const notMinter = Address.parse(process.env.NOT_MINTER!);
    const pxMinter = Address.parse(process.env.PX_MINTER!);
    const dogsMinter = Address.parse(process.env.DOGS_MINTER!);
    ...
    const usdtJettonWalletCode = Cell.fromHex(jettonWalletCode);
    const grcJettonWalletCode = Cell.fromHex(jettonWalletCode);
    const notJettonWalletCode = Cell.fromHex(jettonWalletCode);
    const pxJettonWalletCode = Cell.fromHex(jettonWalletCode);
